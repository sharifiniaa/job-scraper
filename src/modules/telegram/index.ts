...
export const sendJobToChannel = async (job: Job, count: number, total: number) => {
  try {
    ...
    const message = `<b>🌟${job.title}</b>\n\nCompany: ${job.company}\n\nLocation: ${companyLogoMapper(
      job.location as string,
    )}\n\n📆Date: <i>${convertedDate.toLocaleDateString() + ' ' + convertedDate.toLocaleTimeString()}</i></b>\n${
      job.description ? `\n🔹Description: <i>${job.description}</i>\n\n` : ''
    }\nSource:${job.source === 'linkedin' ? '<strong>LinkedIn</strong>' : ''}\n${job.visa ? '<u>🌐#visa</u>\n' : ''}`;
    const keyboard = {
